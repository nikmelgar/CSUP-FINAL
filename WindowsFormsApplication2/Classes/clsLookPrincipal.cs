    ...
    class clsLookPrincipal
    {
        ...
        public void loaddefaultitems(DataGridView dgv)
        {
            using (SqlConnection con = new SqlConnection(global.connectString()))
            {
                ...
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT top 50 * FROM membership where Date_Resigned_From_Pecci is null and isActive = '1' and isApprove = '1' and Principal = '1'", con);
        ...
        public void searchPrincipal(string employeeid, string firstname, string lastname, DataGridView dgv)
        {
            using (SqlConnection con = new SqlConnection(global.connectString()))
            {
                con.Open();

                SqlDataAdapter adapter = new SqlDataAdapter("SELECT top 50 * FROM membership where Date_Resigned_From_Pecci is null and EmployeeID like '%" + employeeid + "%' and LastName like '%" + lastname + "%' and FirstName like '%" + firstname + "%' and isActive = '1' and isApprove = '1' and Principal = '1'", con);
                DataTable dt = new DataTable();
                adapter.Fill(dt);
